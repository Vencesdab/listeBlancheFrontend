...
export class ProfilService {
  ...
  putPassword(password: string) {
	...
	return this.http.put(address,{password: password},{withCredentials: !config["proxy-dev"]})
  }

  putEmailPassword(password: string) {
	const address = config.url + 'api/user/email_password'
	return this.http.put(address,{email_password: password},{withCredentials: !config["proxy-dev"]})
  }
}